...
const TAB_LABELS: Record<InstructorNavTab, string> = {
  overview: 'Dashboard', my_classes: 'My Classes', schedule: 'My Schedule', attendance: 'Attendance',
...
const NOTIF_DOT: Record<string, string> = {
  grade: 'var(--brand-gold)', attendance: 'var(--status-warning)',
  schedule: 'var(--status-info)', enrollment: 'var(--status-success)',
  announcement: 'var(--accent-gold-hover)', system: 'var(--text-faint)',
};
      ...
      <div className="flex justify-between items-center w-full px-4 sm:px-6 py-3">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <nav className="hidden sm:flex items-center gap-1.5 text-xs lg:text-sm" style={{ color: 'var(--text-muted)' }} aria-label="Breadcrumb">
            <button onClick={() => setActiveTab('overview')} className="hover:text-[--brand-gold] transition-colors font-medium">Instructor</button>
        ...
        {/* Right controls */}
        <div className="flex items-center gap-2 sm:gap-3">
          <span className="hidden lg:block font-mono text-xs" style={{ color: 'var(--text-faint)' }}>{today}</span>
          <Badge variant="gold" className="hidden sm:inline-flex">{profile.currentSemester}</Badge>

          <button onClick={onOpenSearch}
            className="hidden md:flex items-center justify-between gap-3 px-3.5 py-1.5 ds-search border rounded-full text-xs w-44 lg:w-52 transition-all"
