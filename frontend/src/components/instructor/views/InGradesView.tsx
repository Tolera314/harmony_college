...
export const InGradesView: React.FC<InGradesViewProps> = ({ programType }) => {
  ...
  // ── Load classes ─────────────────────────────────────────────────────────
  const loadClasses = useCallback(async (keepSelection = false) => {
    try {
      ...
      if (data.length > 0) {
        ...
        setSelectedOffering((prev) => {
          if (keepSelection && prev && data.some((d) => d.id === prev)) return prev;
          return current ? current.id : data[0].id;
        });
      } else {
        setSelectedOffering('');
      }
  ...
  // Refresh class list (to update per-course submission statuses) without changing selection
  ...
  // Switch to a different course offering — reset local marks so previous course data is not shown
  const handleSelectCourse = (offeringId: string) => {
    if (offeringId === selectedOffering) return;
    setLocalMarks({});
    setGrades([]);
    setError(null);
  ...
  return (
    ...
    >
      <DHPageHeader
        ...
        subtitle="Enter and manage assessment marks for your assigned courses."
      ...
      {/* ── Course Switcher (only shown when teacher has 2+ courses) ──────────── */}
      {classes.length > 1 && (
        <div className="flex flex-wrap gap-2">
          {classes.map((c) => {
            const isActive = c.id === selectedOffering;
            const st = c.submissionStatus ?? 'PENDING';
            const statusColor =
              st === 'PUBLISHED'
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-xs font-mono font-semibold transition-all duration-150 ${
                  isActive
                    ? 'bg-[#E9C349]/15 border-[#E9C349]/50 text-[#E9C349] shadow-sm shadow-[#E9C349]/10'
              ...
              >
                ...
                <span className="hidden sm:inline font-normal">{c.course.name}</span>
                <span className={`ml-1 px-1.5 py-0.5 rounded-md border text-[9px] font-bold uppercase tracking-wider ${statusColor}`}>
                  {statusLabel}
                </span>
              </button>
            );
      ...
      {/* ── Unified Course Info Card ─────────────────────────────────────────── */}
      {selectedClass && (() => {
        const st = selectedClass.submissionStatus ?? 'PENDING';
        const statusCfg = {
          PUBLISHED:   { cls: 'bg-sky-500/15 border-sky-400/40 text-sky-300',          label: 'PUBLISHED' },
          SUBMITTED:   { cls: 'bg-emerald-500/15 border-emerald-400/40 text-emerald-300', label: 'SUBMITTED' },
          IN_PROGRESS: { cls: 'bg-amber-500/15 border-amber-400/40 text-amber-300',     label: 'IN PROGRESS' },
          PENDING:     { cls: 'bg-zinc-500/10 border-zinc-500/30 text-zinc-400',        label: 'PENDING' },
        }[st];
        return (
          <div className="p-4 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            {/* Left: identity */}
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-xl bg-[#E9C349]/15 border border-[#E9C349]/30 flex items-center justify-center font-mono font-bold text-xs text-[#E9C349] shrink-0">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className={`px-2 py-0.5 rounded-md border text-[10px] font-mono font-bold uppercase tracking-wider ${statusCfg.cls}`}>
                    {statusCfg.label}
                  </span>
                </div>
                <p className="text-[11px] font-mono text-zinc-400 mt-0.5">
                  Section {selectedClass.section} · {selectedClass.semester.name}
                  {' '}· ECTS: <strong className="text-[#E9C349]">{courseEcts}</strong>
                  {' '}· Credit Hours: <strong className="text-zinc-200">{courseCreditHours}</strong>
                </p>
              </div>
            </div>
            {/* Right: meta chips */}
            <div className="flex items-center gap-2 text-[11px] font-mono shrink-0">
              <span className="px-2.5 py-1 rounded-lg bg-black/30 border border-white/10 text-zinc-400">
              ...
              {isEditingOpen ? (
                ...
                <span className="px-2.5 py-1 rounded-lg bg-rose-500/10 border border-rose-500/30 text-rose-400 font-bold flex items-center gap-1">
                  <Lock className="w-3 h-3" /> Grade Editing Closed
                </span>
              )}
            </div>
          </div>
