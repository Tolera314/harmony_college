...
const typeConfig: Record<HRNotification['type'], { dot: string; badge: 'gold'|'amber'|'rose'|'emerald'|'glass' }> = {
  leave:       { dot: 'bg-amber-400',   badge: 'amber' },
  payroll:     { dot: 'bg-[#E9C349]',   badge: 'gold' },
  performance: { dot: 'bg-sky-400',     badge: 'glass' },
  contract:    { dot: 'bg-rose-400',    badge: 'rose' },
  onboarding:  { dot: 'bg-emerald-400', badge: 'emerald' },
          ...
          return (
            <motion.div
              ...
              className={`flex items-start gap-4 p-4 rounded-2xl border cursor-pointer transition-all ${
                !n.read ? 'bg-white/[0.04] border-white/15 hover:bg-white/[0.07]' : 'bg-transparent border-white/8 hover:bg-white/5'
              }`}
            >
