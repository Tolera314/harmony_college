  ...
  return (
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ ...DURATION.medium, ...EASE.out }} className="space-y-6 pb-16">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select value={methodFilter} onChange={e => setMethodFilter(e.target.value)} className="px-3 py-2 bg-(--hover-overlay) border border-(--border-default) rounded-xl font-sans text-xs text-(--text-secondary) focus:outline-none focus:border-(--brand-gold)">
          {['All', 'Chapa', 'Telebirr', 'Bank Transfer', 'Cash', 'Commercial Bank'].map(m => <option key={m} className="bg-(--bg-card-solid)" value={m}>{m}</option>)}
        </select>
      </div>
          ...
          <tbody className="divide-y divide-(--border-subtle)">
            {filtered.map(p => (
              <tr key={p.id} className="hover:bg-(--hover-overlay) transition-colors">
                <td className="px-4 py-3.5 font-mono text-xs text-(--brand-gold)">{p.transactionId}</td>
                <td className="px-4 py-3.5 font-semibold text-(--text-primary)">{p.studentName}</td>
                <td className="px-4 py-3.5 font-mono text-sm font-bold text-(--text-primary)">ETB {p.amount.toLocaleString()}</td>
                <td className="px-4 py-3.5"><Badge variant="glass" className="text-[10px]">{p.method}</Badge></td>
                <td className="px-4 py-3.5 text-(--text-secondary)">{p.category}</td>
                <td className="px-4 py-3.5 font-mono text-xs text-(--text-muted)">{p.date}</td>
