    ...
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ ...DURATION.medium, ...EASE.out }} className="space-y-6 pb-16">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select value={statusFilter} onChange={e => { setStatusFilter(e.target.value as typeof statusFilter); setPage(1); }} className="px-3 py-2 bg-(--hover-overlay) border border-(--border-default) rounded-xl font-sans text-xs text-(--text-secondary) focus:outline-none focus:border-(--brand-gold)">
          {['All', 'Active', 'Graduated', 'Suspended', 'Dropped', 'On Leave'].map(s => <option key={s} className="bg-(--bg-card-solid)" value={s}>{s}</option>)}
        </select>
      </div>
      <div className="overflow-x-auto border border-(--border-default) rounded-2xl bg-(--hover-overlay) backdrop-blur-xl">
        <table className="w-full text-left text-xs font-sans min-w-[900px]">
          ...
          <tbody className="divide-y divide-(--border-subtle)">
            {paginated.length === 0 ? <tr><td colSpan={8} className="p-0"><EmptyState variant="students" compact /></td></tr>
            : paginated.map(s => {
                ...
                <tr key={s.id} className="hover:bg-(--hover-overlay) transition-colors">
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div><p className="font-semibold text-(--text-primary)">{s.name}</p><p className="font-mono text-[10px] text-(--text-faint)">{s.studentId}</p></div>
                    </div>
                  </td>
                  <td className="px-4 py-3.5 text-(--text-secondary) max-w-[140px] truncate">{prog?.code ?? '—'}</td>
                  ...
                  <td className="px-4 py-3.5"><span className={`font-mono text-xs font-semibold ${s.attendanceRate >= 90 ? 'text-(--status-success)' : s.attendanceRate >= 80 ? 'text-(--brand-gold)' : 'text-(--status-danger)'}`}>{s.attendanceRate}%</span></td>
                  <td className="px-4 py-3.5"><span className={`font-mono text-xs font-semibold ${s.tuitionBalance > 0 ? 'text-(--status-danger)' : 'text-(--status-success)'}`}>{s.tuitionBalance > 0 ? `ETB ${s.tuitionBalance.toLocaleString()}` : 'Cleared'}</span></td>
