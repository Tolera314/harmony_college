...
import { LayoutDashboard, Users, Shield, BarChart3, Settings, Bell } from 'lucide-react';
...
export const AdminMobileNav: React.FC<AdminMobileNavProps> = ({ activeTab, setActiveTab, unreadCount }) => {
  const items: { id: AdminNavTab; label: string; icon: React.ReactNode; dot?: boolean }[] = [
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden p-3 pointer-events-none">
      <nav className="pointer-events-auto max-w-lg mx-auto bg-[var(--bg-panel)]/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl flex items-center justify-around px-2 py-1.5">
        {items.map(item => {
          ...
          return (
            <motion.button key={item.id} onClick={() => setActiveTab(item.id)} whileTap={{ scale: 0.88 }} aria-current={isActive ? 'page' : undefined}
              ...
              {isActive && <motion.div layoutId="adminMobilePill" className="absolute inset-0 bg-[#E9C349]/15 rounded-xl border-b-2 border-[#E9C349]" transition={{ type: 'spring', stiffness: 400, damping: 30 }} />}
