...
import { Search, X, Users, GraduationCap, BookOpen, DollarSign, Shield } from 'lucide-react';
import { AdminNavTab } from '../../types/admin';
import { systemUsers, adminStudents, programs } from '../../data/adminData';
...
interface Result { id: string; label: string; sub: string; tab: AdminNavTab; icon: React.ReactNode }
...
export const AdminSearchModal: React.FC<AdminSearchModalProps> = ({ isOpen, onClose, onNavigate }) => {
  ...
  const allResults: Result[] = [
    ...systemUsers.map(u => ({ id: u.id, label: u.name, sub: `${u.userId} · ${u.role}`, tab: 'users' as AdminNavTab, icon: <Users className="w-4 h-4" /> })),
    ...adminStudents.map(s => ({ id: s.id, label: s.name, sub: `${s.studentId} · ${s.programId}`, tab: 'students' as AdminNavTab, icon: <GraduationCap className="w-4 h-4" /> })),
    ...programs.map(p => ({ id: p.id, label: p.name, sub: `${p.code} · ${p.level}`, tab: 'programs' as AdminNavTab, icon: <BookOpen className="w-4 h-4" /> })),
  ];
  ...
  const quickLinks: { tab: AdminNavTab; label: string; icon: React.ReactNode }[] = [
    ...
    { tab: 'reports',  label: 'Reports',       icon: <Search className="w-4 h-4" /> },
  ];

  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 backdrop-blur-md z-50 flex items-start justify-center pt-[12vh] px-4" style={{ backgroundColor: "var(--overlay-modal-bg)" }} onClick={onClose}>
          <motion.div
          ...
          >
            <div className="flex items-center gap-3 px-4 py-3.5 border-b ds-search-modal-bar">
              ...
              <input ref={inputRef} type="text" value={query} onChange={e => setQuery(e.target.value)}
                placeholder="Search users, students, programs, reports..."
                className="flex-1 ds-search-modal-input font-sans text-sm outline-none" aria-label="Global admin search" />
