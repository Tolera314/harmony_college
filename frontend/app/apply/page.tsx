...
type FieldErrors = Partial<Record<keyof AppFormData, string>>;
...
// ─── InputField ──────────────────────────────────────────────────────────────
function InputField({ icon: Icon, label, type = 'text', required = false, value, onChange, onBlur, readOnly = false, error }: {
  ...
  return (
    <div className="relative group mb-5">
      ...
      <input
        type={type} required={required} value={value} onChange={onChange} onBlur={onBlur} readOnly={readOnly}
        className={`w-full bg-black/40 border rounded-xl py-3.5 pl-12 pr-4 text-white text-sm focus:outline-none focus:ring-1 transition-all peer placeholder-transparent disabled:opacity-60
      ...
      {error && (
        <p className="mt-1 ml-1 text-[11px] text-red-400 flex items-center gap-1">
...
function ApplyPageInner() {
  ...
  // Per-field live validation errors
  const [fieldErrors, setFieldErrors] = useState<FieldErrors>({});

  const setFieldError = (key: keyof AppFormData, msg: string) =>
    setFieldErrors((p) => ({ ...p, [key]: msg }));
  const clearFieldError = (key: keyof AppFormData) =>
    setFieldErrors((p) => { const n = { ...p }; delete n[key]; return n; });

  // Profile image state
  const [profileImage, setProfileImage] = useState<ProfileImageState>({
    file: null, preview: '', uploading: false, error: '',
  });

  const pickProfileImage = (file: File) => {
    if (!['image/jpeg', 'image/png', 'image/jpg', 'image/webp'].includes(file.type)) {
      setProfileImage((p) => ({ ...p, error: 'Only JPG, PNG or WEBP images are allowed.' }));
      return;
    }
    if (file.size > 2 * 1024 * 1024) {
      setProfileImage((p) => ({ ...p, error: 'Image must be under 2 MB.' }));
      return;
    }
    const preview = URL.createObjectURL(file);
    setProfileImage({ file, preview, uploading: false, error: '' });
  };
  ...
  const update = (key: keyof AppFormData) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    setFormData((p) => ({ ...p, [key]: e.target.value }));
    if (e.target.value) clearFieldError(key);
  };

  const set = (key: keyof AppFormData, val: string) =>
  ...
  const uploadDoc = async (key: keyof typeof docs, file: File) => {
    ...
    try {
      ...
      setDocs((p) => ({ ...p, [key]: { file, url: '', uploading: false, error: msg } }));
    }
  };
  ...
  const validateStep = (): boolean => {
    ...
    if (step === 1) {
      ...
      if (!formData.dob) errors.dob = 'Date of birth is required.';
        ...
        if (!formData.age || isNaN(age) || age < 15) errors.dob = 'Applicant must be at least 15 years old.';
      }
    }
    if (step === 2) {
      if (!formData.nationality) errors.nationality = 'Nationality is required.';
      if (!formData.phone.trim()) errors.phone = 'Phone number is required.';
      else if (!/^(?:\+251|0)[79]\d{8}$/.test(formData.phone)) errors.phone = 'Enter a valid Ethiopian phone (09XX... or +251...).';
      if (!formData.emergencyContact.trim()) errors.emergencyContact = 'Emergency contact is required.';
      else if (!/^(?:\+251|0)[79]\d{8}$/.test(formData.emergencyContact)) errors.emergencyContact = 'Enter a valid Ethiopian phone.';
    ...
    if (step === 3) {
      ...
      if (!formData.academicYear) errors.academicYear = 'Please select an academic year.';
    }
    ...
    if (step === 4) {
      if (!docs.matric.url) { setSubmitError('Grade 12 / Matric document is required.'); return false; }
      if (!docs.grade8.url) { setSubmitError('Grade 8 / Ministry document is required.'); return false; }
      if (!docs.transcript910.url) { setSubmitError('Transcript 9–10 is required.'); return false; }
      if (!docs.transcript1112.url) { setSubmitError('Transcript 11–12 is required.'); return false; }
    }
    if (step === 5) {
      if (!consents.certify) { setSubmitError('Please certify that all information is accurate.'); return false; }
      if (!consents.falseInfo) { setSubmitError('Please acknowledge the false information policy.'); return false; }
      if (!consents.admissionPolicy) { setSubmitError('Please agree to the Admission Policy.'); return false; }
      if (!consents.privacyPolicy) { setSubmitError('Please consent to the Privacy Policy.'); return false; }
    }
    return true;
  };
  ...
  const prevStep = () => { setSubmitError(''); setFieldErrors({}); if (step > 1) setStep((s) => s - 1); };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        // Surface individual field errors from Zod details when present
        if (data.details && typeof data.details === 'object') {
          const fieldMsgs = Object.entries(data.details as Record<string, string[]>)
            .map(([field, msgs]) => `${field}: ${msgs[0]}`)
            .join(' · ');
          throw new Error(fieldMsgs || data.error || 'Validation failed.');
        }
        ...
        >
          ...
          {!isSuccess ? (
            <>
              ...
              {/* FORM AREA */}
              <form onSubmit={nextStep} className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] pr-2 flex flex-col">
                  ...
                  {step === 1 && (
                    <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-2 pb-4">
                      ...
                      <div className="flex items-center gap-6 mb-8 bg-white/5 p-4 rounded-xl border border-white/5">
                        <div
                          className="w-20 h-20 rounded-full bg-black/50 border-2 border-dashed border-white/20 flex flex-col items-center justify-center text-gray-500 hover:border-[#D4AF37] hover:text-[#D4AF37] transition-all cursor-pointer relative group shrink-0 overflow-hidden"
                        ...
                        >
                          ...
                          <input
                            ...
                            onChange={(e) => { if (e.target.files?.[0]) pickProfileImage(e.target.files[0]); }}
                          />
                          {profileImage.preview && (
                        ...
                        <div>
                          ...
                          {profileImage.file && !profileImage.error && (
                            <p className="text-xs text-green-400 mt-1 flex items-center gap-1">
                              <CheckCircle2 className="w-3 h-3" /> {profileImage.file.name}
                            </p>
                          )}
                          {profileImage.error && (
                            <p className="text-xs text-red-400 mt-1 flex items-center gap-1">
                              <AlertCircle className="w-3 h-3" /> {profileImage.error}
                            </p>
                          )}
                        </div>
                      </div>
                      ...
                      <InputField icon={User} label="Full Name" required value={formData.fullName}
                        ...
                        onBlur={() => {
                          if (!formData.fullName.trim()) setFieldError('fullName', 'Full name is required.');
                  ...
                  {step === 2 && (
                    <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-2 pb-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <InputField icon={Phone} label="Phone Number" type="tel" required value={formData.phone}
                          onChange={update('phone')}
                          onBlur={() => {
                            if (!formData.phone.trim()) setFieldError('phone', 'Phone number is required.');
                            else if (!/^(?:\+251|0)[79]\d{8}$/.test(formData.phone)) setFieldError('phone', 'Enter a valid Ethiopian phone (09XX... or +251...).');
                            else clearFieldError('phone');
                          }}
                          error={fieldErrors.phone}
                        />
                        <InputField icon={Phone} label="Emergency Contact" type="tel" required value={formData.emergencyContact}
                          onChange={update('emergencyContact')}
                          onBlur={() => {
                            if (!formData.emergencyContact.trim()) setFieldError('emergencyContact', 'Emergency contact is required.');
                            else if (!/^(?:\+251|0)[79]\d{8}$/.test(formData.emergencyContact)) setFieldError('emergencyContact', 'Enter a valid Ethiopian phone.');
                            else clearFieldError('emergencyContact');
                          }}
                          error={fieldErrors.emergencyContact}
                        />
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <InputField icon={Mail} label="Email Address" type="email" required value={formData.email}
                          onChange={update('email')}
                          onBlur={() => {
                            if (!formData.email.trim()) setFieldError('email', 'Email is required.');
                            else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) setFieldError('email', 'Enter a valid email address.');
                            else clearFieldError('email');
                          }}
                          error={fieldErrors.email}
                        />
                        <InputField icon={Lock} label="Account Password" type="password" required value={formData.password}
                          onChange={update('password')}
                          onBlur={() => {
                            if (!formData.password) setFieldError('password', 'Password is required.');
                            else if (formData.password.length < 6) setFieldError('password', 'Minimum 6 characters.');
                      ...
                      <div className="grid grid-cols-2 gap-4 mt-1">
                        ...
                        <InputField icon={MapPin} label="Manual Address" required value={formData.address}
                          onChange={update('address')}
                          onBlur={() => {
                            if (!formData.address.trim()) setFieldError('address', 'Address is required.');
                          ...
                          <div className="space-y-3">
                            <label className="flex items-start gap-3 cursor-pointer group">
                              <div className="relative flex items-center justify-center w-4 h-4 mt-0.5 shrink-0">
                                <input type="checkbox" checked={consents.certify} onChange={() => toggleConsent('certify')}
                                  className="peer appearance-none w-4 h-4 border border-gray-600 rounded-[3px] checked:bg-[#D4AF37] checked:border-[#D4AF37] transition-all cursor-pointer" />
                              ...
                              <span className="text-xs text-gray-300 group-hover:text-white transition-colors">I certify that all information provided is accurate.</span>
                            </label>
                            <label className="flex items-start gap-3 cursor-pointer group">
                              <div className="relative flex items-center justify-center w-4 h-4 mt-0.5 shrink-0">
                                <input type="checkbox" checked={consents.falseInfo} onChange={() => toggleConsent('falseInfo')}
                                  className="peer appearance-none w-4 h-4 border border-gray-600 rounded-[3px] checked:bg-[#D4AF37] checked:border-[#D4AF37] transition-all cursor-pointer" />
                                <svg className="absolute w-3 h-3 text-[#0F0F10] opacity-0 peer-checked:opacity-100 pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>
                              </div>
                              <span className="text-xs text-gray-300 group-hover:text-white transition-colors">I understand that false information may result in rejection.</span>
                            </label>
                          </div>
                        </div>
                        <div>
                          ...
                          <div className="space-y-3">
                            <label className="flex items-start gap-3 cursor-pointer group">
                              <div className="relative flex items-center justify-center w-4 h-4 mt-0.5 shrink-0">
                                <input type="checkbox" checked={consents.admissionPolicy} onChange={() => toggleConsent('admissionPolicy')}
                                  className="peer appearance-none w-4 h-4 border border-gray-600 rounded-[3px] checked:bg-[#D4AF37] checked:border-[#D4AF37] transition-all" />
                                <svg className="absolute w-3 h-3 text-[#0F0F10] opacity-0 peer-checked:opacity-100 pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>
                              </div>
                              <span className="text-xs text-gray-300">I agree to the <span className="text-[#D4AF37]">Admission Policy</span>.</span>
                            </label>
                            <label className="flex items-start gap-3 cursor-pointer group">
                              <div className="relative flex items-center justify-center w-4 h-4 mt-0.5 shrink-0">
                                <input type="checkbox" checked={consents.privacyPolicy} onChange={() => toggleConsent('privacyPolicy')}
                                  className="peer appearance-none w-4 h-4 border border-gray-600 rounded-[3px] checked:bg-[#D4AF37] checked:border-[#D4AF37] transition-all" />
                                <svg className="absolute w-3 h-3 text-[#0F0F10] opacity-0 peer-checked:opacity-100 pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>
                              </div>
                              <span className="text-xs text-gray-300">I consent to data processing per the <span className="text-[#D4AF37]">Privacy Policy</span>.</span>
                            </label>
                          </div>
                        </div>
                      </div>
